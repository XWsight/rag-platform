...
class ApplicationService:
    ...
    def create_project(self, principal: Principal, display_name: str, description: str = "") -> Project:
        _require_writer(principal)
        now = self._now()
        project = Project(
            project_id=_new_id("prj"), tenant_id=principal.tenant_id,
            display_name=display_name, description=description, created_at=now, updated_at=now,
        )
        ...
        self._repository.record_audit_event(
            ...
            AuditEvent(
                ...
                event_type=ApplicationAuditEventType.PROJECT_CREATED, occurred_at=now,
                actor=principal.subject, summary="Created a project.", project_id=created.project_id,
            ),
        )
        return created
    ...
    def create_knowledge_application(
        ...
        now = self._now()
        application = Application(
            application_id=_new_id("app"), tenant_id=principal.tenant_id, project_id=project_id,
            display_name=display_name, application_kind=ApplicationKind.KNOWLEDGE_CHAT,
            active_revision_id=None, status=ApplicationStatus.ACTIVE, created_at=now, updated_at=now,
        )
        ...
        self._repository.record_audit_event(
            ...
            AuditEvent(
                audit_event_id=_new_id("audit"), tenant_id=principal.tenant_id,
                event_type=ApplicationAuditEventType.APPLICATION_CREATED, occurred_at=now,
                actor=principal.subject, summary="Created a knowledge-chat application.",
                project_id=created.project_id, application_id=created.application_id,
            ),
        )
        return created
    ...
    def create_knowledge_revision(
        ...
        *,
        ...
        now = self._now()
        revisions = self._repository.list_revisions(principal, application.application_id, limit=100)
        revision = ApplicationRevision(
            revision_id=_new_id("rev"), application_id=application.application_id,
            ...
            configuration=configuration, created_at=now, created_by=principal.subject,
            change_summary=change_summary,
        )
        bindings = tuple(
            ResourceBinding(
                binding_id=_new_id("bind"), application_id=application.application_id,
                revision_id=revision.revision_id, resource_kind=ResourceKind.KNOWLEDGE_BASE,
                resource_id=resource_id, access_mode=ResourceAccessMode.READ, created_at=now,
            )
        ...
        self._repository.record_audit_event(
            ...
            AuditEvent(
                audit_event_id=_new_id("audit"), tenant_id=principal.tenant_id,
                event_type=ApplicationAuditEventType.REVISION_CREATED, occurred_at=now,
                actor=principal.subject, summary="Created an immutable application revision.",
    ...
    def publish(
        ...
        *,
        ...
        now = self._now()
        deployment = Deployment(
            deployment_id=_new_id("dep"), application_id=application.application_id,
            revision_id=revision.revision_id, environment=environment, deployed_at=now,
            deployed_by=principal.subject,
        )
        event = AuditEvent(
            audit_event_id=_new_id("audit"), tenant_id=principal.tenant_id,
            event_type=ApplicationAuditEventType.DEPLOYMENT_CREATED, occurred_at=now,
            actor=principal.subject, summary="Published an application revision.",
            project_id=application.project_id, application_id=application.application_id,
    ...
    def _verify_ready_knowledge_bases(self, principal: Principal, resource_ids: Sequence[str]) -> None:
