...
from scripts.migrate_legacy_volume import VolumeMigrationError, build_plan, execute_plan


class VolumeMigrationPlanTests(unittest.TestCase):
    def test_plan_is_non_destructive_until_explicitly_executed(self) -> None:
        plan = build_plan(
            source_volume="rag-studio-data",
            destination_volume="rag-platform-data",
        )

        self.assertEqual(plan.create_command, ("docker", "volume", "create", "rag-platform-data"))
        self.assertIn("rag-studio-data:/source:ro", plan.copy_command)
    ...
    def test_plan_rejects_unsafe_or_overwriting_volume_names(self) -> None:
        for source, destination in (
            ("rag-studio-data", "rag-studio-data"),
            ("rag-studio-data", "other:/data"),
    ...
    def test_execute_preserves_the_source_and_reports_a_verified_copy(self) -> None:
        ...
        with patch("scripts.migrate_legacy_volume._require_volume") as require, patch(
            "scripts.migrate_legacy_volume._run",
            side_effect=("", "", manifest, manifest),
        ) as run:
            result = execute_plan(plan)

        require.assert_any_call("rag-studio-data", must_exist=True)
        require.assert_any_call("rag-platform-data", must_exist=False)
        self.assertEqual(run.call_count, 4)
        self.assertEqual(result["file_count"], 2)
        self.assertTrue(result["verified"])

    def test_execute_keeps_the_destination_for_manual_inspection_after_a_mismatch(self) -> None:
        plan = build_plan(
            source_volume="rag-studio-data",
            destination_volume="rag-platform-data",
        )
        with patch("scripts.migrate_legacy_volume._require_volume"), patch(
            "scripts.migrate_legacy_volume._run",
