...
`applications.sqlite3` 当前 schema 为 v8；升级会在事务内依次补齐云端策略默认值、扩展退役/Draft 审计事件，为旧版本配置写入显式的 `retrieval_profile=default` 和 `model_profile_id=default`，为每个应用创建空 Draft，并建立不可变 Revision 的评测证据表。最后一步会把当前 Application 指针对应的最新 Deployment 标为 `active`，其余历史记录标为 `superseded`，并以部分唯一索引保证每个应用最多一个激活记录。升级前仍须停写并备份整个持久卷；回滚必须同时恢复旧镜像和升级前快照。
