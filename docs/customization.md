...
RAG Platform 是可派生的通用知识检索基座，而不是某个行业的固定应用。派生项目应保留其安全、租户、证据和评测契约，在明确的扩展点中增加行业能力；不要复制后在业务代码中散落修改核心边界。

如需创建可提交、可同步上游的派生层，请使用[派生项目脚手架](derivative-scaffold.md)，而不是复制并修改核心运行时。
