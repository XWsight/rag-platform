...
- 新增 v1 OpenAPI 契约快照和逐字校验器；API 改动必须在 PR 中明确审阅并更新基线，版本号本身不会制造无意义漂移。质量工作流新增稳定 `quality-gate` 汇总结果，供分支保护长期引用。
- 新增可执行的 `RELEASE.md`：明确 SemVer、v1 API/持久化兼容性边界、稳定 tag、镜像证据验证与基于 digest 的回滚流程。
